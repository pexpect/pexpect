...
class ExpectTestCase(PexpectTestCase.PexpectTestCase):
    ...
    def _expect_order(self, p):
        ...
        index = p.expect(
            [b"54321", pexpect.TIMEOUT, b"1234", b"abcd", b"wxyz", pexpect.EOF],
            timeout=5,
        )
        ...
        index = p.expect([b"abcd", b"wxyz", b"7890", pexpect.EOF])
        assert index == 3, (index, p.before, p.after)
    ...
    def test_waitnoecho_order(self):
        ...
        end_time = time.time() - start
        assert end_time > 3, (
            "waitnoecho should have waited longer than 2 seconds. retval should be False, retval=%d"
            % retval
        )
    ...
    def _expect_echo(self, p):
        p.sendline(
            b"1234"
        )  # Should see this twice (once from tty echo and again from cat).
    ...
    def _expect_index(self, p):
        ...
        p.sendline(b"wxyz")
        index = p.expect(
            [b"54321", pexpect.TIMEOUT, b"abcd", b"wxyz", b"1234", pexpect.EOF]
        )
    ...
    def test_before_across_chunks(self):
        ...
        child = pexpect.spawn(
            '''/bin/bash -c "openssl rand -base64 {} 2>/dev/null | head -500 | nl --number-format=rz --number-width=5 2>&1 ; echo 'PATTERN!!!'"'''.format(
                1024 * 1024 * 2
            ),
    ...
    def test_before_after_timeout(self):
        ...
        e = child.expect(
            [b"xyzzy", pexpect.TIMEOUT], searchwindowsize=10, timeout=0.001
        )
    ...
    def test_increasing_searchwindowsize(self):
        ...
        self.assertEqual(e, 1)
        e = child.expect(
            [b"BEGIN", pexpect.TIMEOUT], searchwindowsize=40000, timeout=30.0
        )
        self.assertEqual(e, 0)
        child.sendeof()
        child.expect(pexpect.EOF)
