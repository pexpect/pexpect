...
class SSHTestBase(PexpectTestCase):
    def setUp(self):
        ...
        fakessh_dir = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "fakessh")
        )
        os.environ["PATH"] = (
            self.tempdir
            + os.pathsep
...
class PxsshTestCase(SSHTestBase):
    ...
    def test_failed_set_unique_prompt(self):
        ...
        try:
            ssh.login("server", "me", password="s3cret", auto_prompt_reset=True)
        except pxssh.ExceptionPxssh:
            pass
        else:
    ...
    def test_username_or_ssh_config(self):
        try:
            ...
            raise AssertionError(
                "Should have failed due to missing username and missing ssh_config."
            )
        except TypeError:
            pass
    ...
    def test_ssh_key_string(self):
        ...
        if confirmation_strings != len(confirmation_array):
            assert (
                False
            ), "String generated from forcing the SSH agent sock is incorrect."
    ...
    def test_custom_ssh_cmd_debug(self):
        ...
        string = ssh.login(
            "server", "me", password="s3cret", cmd="ssh " + cipher_string + " -2"
        )
        for confirmation in confirmation_array:
            if confirmation in string:
                confirmation_strings += 1
    ...
    def test_failed_custom_ssh_cmd(self):
        try:
            ...
            if "Closed connection" not in string:
                assert (
                    False
                ), "should not have completed logging into Mock SSH client and exited"
        except pxssh.ExceptionPxssh as e:
            pass
        else:
            assert False, "should have raised exception, pxssh.ExceptionPxssh"
